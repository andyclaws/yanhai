  ...
  <div class="tableRight"
    ...
    <div>
      <Tables border
              ...
              :axiosData="axiosData"
              class="table" />
      <Modals v-model="show_sonTables" @on-cancel="show_sonTables = false" title="台区明细" width='65' footer-hide>
        <sonTables ref="sonTables" :axiosUrl="sonAxiosUrl" :axiosData="sonAxiosData" @cancel="show_sonTables = false"/>
      </Modals>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    queryBottomTable (params, name) {
      this.sonAxiosUrl = '/sea-lineLose/unitControl/trendAnalysis/' + name
      this.sonAxiosData = {}
      if (name.indexOf('continue') !== -1) {
        this.sonAxiosData = {
          'orgNo': params.row.orgNo,
          'orgType': params.row.orgType,
      ...
      this.$nextTick(() => {
        this.$refs['sonTables'].$refs.tables.queryTableData()
      })
