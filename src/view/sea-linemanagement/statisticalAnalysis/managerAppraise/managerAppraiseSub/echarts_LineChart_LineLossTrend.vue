...
export default {
  ...
  methods: {
    drawLineChart () {
      ...
      this.lineChartYdata = this.lineChartDataList.map(_ => _.lllr)
      var linevalue = 20
    ...
    publicFunction () {
      ...
      axios.request({
        ...
        url: '/sea-lineLose/custManagerAssess/queryLineLossTrend'
      }).then(response => {
        this.lineChartDataList = response.data.data.list
        this.$nextTick(() => {
          this.drawLineChart()
        })
      })
