...
import loginAxios from '@/libs/api.login'
  ...
  methods: {
    ...
    handleColumns (columns) {
      ...
      this.insideColumns = columns.map((item, index) => {
        ...
        if (!res.renderHeader) {
          item.renderHeader = function (createElement, { column }) {
    ...
    exportFile (name) {
      this.showMsg(
        name === 'exportTotalExcel'
          ? '正在为您导出全量，等待时间可能会长一些，请您耐心等待！'
          : undefined
      )
      setTimeout(() => {
        switch (name) {
          ...
          case 'exportTotalExcel':
            ...
            this.loading = true
            let ajax = this.isLoginAxios ? loginAxios : axios
            ajax
              .request({
      ...
      ajax
        ...
        .then(res => {
          ...
          if (res.status === 200 && data.data) {
            ...
            // this.$Message.error('后台程序异常, 查询失败!')
          }
        })
        .catch(() => {
